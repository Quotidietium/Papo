...
## 补充：混合并发负载（CombinedLoadBench）——交互效应缺口闭合

七场景家族均为孤立测试；真实服=并发混合（实体 tick+红石+玩家进出+行走世界生成+
脏区块 autosave 重叠）。单服务器叠加 500 AI 牛 + 100 环振荡器 + 5 churn 槽位
（100 轮零失败）+ 2 行走 bot + 800 脏区块保活，10 分钟窗：**全门 PASS，稳态
