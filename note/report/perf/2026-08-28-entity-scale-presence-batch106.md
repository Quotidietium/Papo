...
| 相位 avg/tick (us) | N=0 | 500 | 1000 | 2000 | 4000 | 每牛 500 | 每牛 1000 | 每牛 2000 | 每牛 4000 |
|---|---|---|---|---|---|---|---|---|---|
| level.entities | 349 | 5053 | 7019 | 19440 | 44062 | 9.4 | 6.7 | **9.5** | **11.0** |
| chunkMap.tracker.fanout | 70 | 9497 | 11897 | 21348 | 2785 | 18.9 | 11.8 | 10.6 | **0.7** |
| chunkMap.tracker.sendChanges | 81 | 9741 | 12249 | 22194 | 4497 | 19.3 | 12.2 | 11.1 | 1.1 |
| chunkMap.tracker.maintain | 27 | 430 | 547 | 891 | 1524 | 0.86 | 0.55 | 0.44 | 0.38 |
| worlds（总） | 1124 | 16255 | 21583 | 44869 | 52039 | — | — | — | — |

关键读数：

1. **容量墙**：2000 聚堆牛 worlds=44.9ms/tick（50ms 预算的 90%），窗墙 20.0s 仍保 20 TPS
   ——贴墙未破；**4000 破墙**：worlds=52.0ms、窗墙 ~21.2s ≈ **TPS 18.9**。
2. **两大面 + jam 相变**：实体 tick 链每牛 9.4→6.7→9.5→**11.0us** U 型持续抬升（密度
   超线性）；扇出 18.9→11.8→10.6us/牛后在 4000 **坍缩至 0.7**——spacing 1.0 时 4000 牛
   挤死平台（游荡寻路无路可走→移动停止→位移包枯竭），同时寻路失败扫描+挤堆推挤灌入
   实体链（44.1ms=worlds 的 84%）。4000 是真实"挤死团"工况（动物农场/村民繁殖器同款），
   但代表性优化区间为 500-2000 流动工况。
3. **maintain 印证批次 101**：0.86→0.44→0.38us/牛单调下降，O(1) purge 全程成立。
4. **扇出单位成本未归因**：19us/牛@500 vs 批次 102 实测 ~90ns/send 差 ~20×——
...
- 4000：boot 即被清扫（21:14）；**21:18 错峰重试全窗存活**（A=4004/B=4007，零死亡零错误，
  +4/+3 杂散同 N=1000 模式）。

## 结论

实体规模轴**在场确立、画像闭合**（0→4000 五点含 jam 相变）：主线程两大牛驱动面为
tracker 扇出（流动工况）与实体 tick 链（全程，jam 工况绝对主导），2000 聚堆牛贴 TPS
预算墙、4000 破墙（TPS 18.9）。R3 深挖次序：①扇出内构成探针（包型×ns/send 归因）→
批量移交/排水优化设计（多核主轴，先例 suspendFlushing/resumeFlushing 已在 Connection）；
②实体链密度超线性子归因（EAR 扫描/goal 邻居查询/jam 寻路失败三候选）；③jam 工况
（挤死团）作为独立画像留待后续批次。harness v5 判例（bot 硬编码起点/population 不受
gamerule 约束/计数式在场门/共享机清扫加固）沉淀入 optimizations.md。
