...
> **2026-09-05 晚间修正（批次125 对照腿发现）**：本报告初版依据的"−20~26% 劣化"
> 在补充同时窗对照后被判定为**环境漂移主导**：批次125 期间用本批次之前的 b123
> 二进制重跑对照腿测得中位 27195（原窗同一二进制 23043，+18% 慢化）——机器在
> ~01:15 后进入持续慢窗（共租负载），b124 两腿（28993/28091）、b125（28250）
> 与对照（27195）全部聚集在 27-29k。**池化的真实效应修正为 +1.3~+3.3%
> （vs 同时代对照），落在噪声带内，无法证实劣化也无法证实收益**。
> 处置维持不变（保守不采纳），但否决理由从"复现的宏劣化"修正为
> "环境噪声下不确定 + JMH 时间中性 + 分配归零不构成采纳充分条件"；若未来
> 重试，须以同时窗交替腿设计进行。下文原始证据链保留（作为环境漂移判例的
> 完整记录）。
