...
## 2026-09-01 补充：批次 118 收尾踩坑（0265 残留，0.71.1 修复）

### git revert 会把"被后续提交空置化"的块原样顶回来

0266 提交在引入 hybrid 等待时曾把 0265 的 timeBeginPeriod 块**原地空置化**（删码留注释）；
随后 `git revert` 0266 时，revert 的语义是"撤销该提交的全部 diff"——把空置化操作也撤销了，
**0265 的原始活代码被完整顶回源码树**。外层回退提交只删了补丁文件（补丁链正确），而
23:54 的重打包编译的是内部源码树 → **产出的 0.71.0 jar 含已声明回退的 0265 代码**（含
boot 期 JNA 调用 + 前后两次 220×parkNanos(1ms) 探针 ≈ 数秒级 boot 阻塞），与补丁链/发布注记
不一致。

**规则（多步回退收尾门）**：
1. revert 后必须 `git diff <最后一个应保留的链上提交> HEAD` 校验内部树——**零差异**才算
   回退闭合（本例锚点=880af8cb/0264，残差恰好 40 行 DedicatedServer 插入）；
2. 修复手段优先"字节级恢复"而非手删：`git show <锚点>:<路径> > <路径>`，再 diff 验证；
3. 声明回退的改动，收尾时对编译产物做字符串级反证（残留串 grep=0 + 阳性对照≥1；
   类在嵌套 jar，见上文批次90判例）——补丁文件正确 ≠ 源码树正确 ≠ 产物正确，三层各自验证。

### 存量手工补丁链期禁全量 rebuildPatches（批次 120 踩坑与恢复路径）

批次 118/119 收尾时补丁目录为**手工维护形态**（链终止 0264+0267，0265/0266 及其
revert/清理四个死提交只存在于内部仓库历史，补丁文件被摘除、编号留空），而内部仓库
HEAD 仍带完整死提交链。此时跑全量 `rebuildPatches` 会**按内部原始历史逐提交再生**，
把四个死补丁复活成 0265-0268 并把 0267→0269、新补丁→0270（总数 265→270），同时
`rebuildSourcePatches` 可能把"新文件式" sources 补丁劣化为"对比式"（Yggdrasil 实例）。

**恢复路径（实测有效）**：
1. 从再生产物中**拷走目标新补丁**（内容自洽，与编号无关）；
2. `git restore --staged paper-server/patches/ && git restore paper-server/patches/`
   整体还原到 HEAD；再清掉 rebuild 遗留的**未跟踪**死补丁文件（restore 不管 untracked）；
3. 目标补丁改号入列（接续现有链的下一空号）；
4. 跑完整 `applyPatches` 重建内部仓库校验链可应用，且重建后内部历史自动甩掉死提交
   （实测重建后历史= 0264→0267→新补丁，紧凑无尸体）。

**规则**：增量补丁时段（存量链为手工形态）一律用"内部仓库手动 commit → format-patch
定向导出/或借 rebuild 单产物 → 手工入列 → applyPatches 校验"，**禁止**顺手全量
rebuildPatches；全量 rebuild 仅在内部历史与补丁链一一对应时使用。
