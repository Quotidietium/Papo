...
## 批次 101（2026-08-27）：tracker purge 探测 O(1) 化——purge 风暴面消除（多核调度系列⑰，0.62.0→0.63.0）

主题：0256 补丁。maintain 超线性归因：逐对成本已紧（~40ns/对，O(N²) 对数在精确性红线下
不可削减——spectator/vanish/lastSendDistance 无完整事件化）；真浪费=**purge 风暴**
（chunk 视图集变化时 160 tracker × 160 seenBy × O(160) 线性 contains，聚堆行走近乎每
...
## 批次 104（2026-08-27）：0.65.0 稳定性回归轮——长时浸泡 + 停机竞态扩样（多核调度系列⑳，验证轮，无代码变更）

主题：批次 97-103 三项优化均触碰实体 tick/追踪路径，回归收口。10bot×10min 浸泡：
12,000 ticks 连续零错误、无上漂、gc 与批次 93 基线同分布；ShutdownRaceVerify ×5 全
PASS（0.59.0 后累计 ×14）——停机窗口无回归。R2 验证矩阵终态：长时+规模+密度面+
封闭面+竞态+每批冒烟全绿。报告：
[note/report/perf/2026-08-27-stability-batch104.md](report/perf/2026-08-27-stability-batch104.md)。

---

## 批次 105（2026-08-28）：实体规模轴勘察——负载生成 harness 迭代与仪器教训（多核调度系列㉑，勘察/基建轮，无服务器代码变更，R2 末轮）

主题：新建 EntityScaleBench 负载生成 harness，四版迭代未确立实体持续在场（spreadplayers
主线程阻塞 5min@500 / EAR 激活半径超出 / 实体 NBT 键名 1.21.x 静默丢弃致坠落全灭 / 选择器
体积陷阱）——**轴量化未达成，如实披露留作开放前沿**；四条仪器判例 + spreadplayers 运营
判例（大 N 禁用）归档。判例：控制台回显格式假设必须实测定字串（"Summoned new Cow" 无
"a"）；stdin 通道可用性以 say 回显实证。报告：
[note/report/perf/2026-08-28-entity-scale-batch105.md](report/perf/2026-08-28-entity-scale-batch105.md)。

---

## R2 循环终止记录（2026-08-28，用户决断）

多核调度 R2 系列（批次 97-105，0.59.0 → 0.65.0）由用户于 2026-08-28 决断"本轮结束后
停止循环"。终态：

- **优化交付**（各带 JMH/等价证明/真服务器 A/B）：99 有界 push 扫描（密度超线性 −32%）、
  100 aiStep 分配消除、101 purge 探测 O(1) 化（风暴 JMH 6.0×，maintain −25~33%）。
- **勘察封闭**：102 投递扇出（语义本体）、103 玩家 tick 链地板（语义载荷）。
- **基建**：97 真并发 harness、98/102/103 相位分解探针族（0253/0257/0258）、105 实体
  负载生成 harness（在场未解，开放）。
- **稳定性**：104 长时浸泡 12k ticks 无上漂 + 停机竞态累计 ×14 全 PASS。
- **开放前沿**：实体规模轴在场问题、红石/流体轴、Folia 级区域化（超红线）。
- 分支 `perf/multicore-r2` 保持未合并（沿用 R1 规则：用户未明示不主动合并）；
  **不发布 GitHub release**（goal 明确禁止，线上 release 停在 0.54.0 属正常）。
...
## 批次 108（2026-08-28）：挂起窗发送批量化——扇出移交成本摊销 12×（多核调度系列㉔，0.66.0→0.67.0）

主题：0260 补丁。批次 107 归因的第一大牛驱动面（每 send 902-1,060ns 移交成本 × 15k
sends/tick）优化：`Connection.send` 即时路径在挂起窗（flush=false 主线程就绪包=tick 内
广播精确形态）append 到每连接批次，单 eventLoop 任务排水；其余路径原样（flush=true
保持逐包直发零延迟变化）。排水点=flushChannel（tick 末与既有 flush 同拍）+直发/入队
FIFO 守卫+256 阈值。等价性六点论证（包集合/序/时序/bundle/失败面/区块发送，见报告）。
在位 A/B（N=2000）：**每 send 902-1,060ns → 75-88ns（~12×），fanout 13.1-16.0ms →
2.0-2.2ms（6-8×），worlds ~45-48ms → 31-35ms（主线程省 ~10-14ms/tick@2000 牛）**；
N=500 后 fanout 159-168us=worlds 的 ~2%。JMH（SendHandoffBench）：无争抢模型 10.3-19.7×，
争抢模型 fork 死于系统页面文件耗尽（DOS 1455 同窗口 test 同因）如实披露，以在位为准。
**首版 NPE 判例**：排水数组复用与在途任务逐槽置 null 竞态 → keepalive 丢失级联断连
（A/B 在场门+bot 全活门抓获）；修复=排水即换新数组（在途任务独占旧数组）。判例：
跨线程交接缓冲必须交接即转移所有权；隔离移交模型与在位口径分歧（30-970ns 入队成本
依赖争抢态）——机制收益以 in-situ ns/send 为准。报告：
[note/report/perf/2026-08-28-sendbatch-batch108.md](report/perf/2026-08-28-sendbatch-batch108.md)。
...
性/兼容性，不为多核牺牲流畅度）并开设新分支 `perf/multicore-r3`（自 perf/multicore-r2
同点，含 R1+R2 全部工作，版本起点 0.65.0）。R3 系列自批次 106 起，R2 终止记录对 R3
...
## 批次 122 — R3 审计轮：跨族交互组合审查 + chunk 包缓存常驻内存驱逐（0.71.3 → 0.71.4，补丁 0269）

整链逐补丁审计于批次 119/120/121 闭环后，本轮转向**跨族交互组合**（0217×0260×0223
叠合出站栈 / 0268×memo 族×0186 混乱 session 逐连接差异源全量再证伪 / 0242 快照性×
k-nearest 输入面 / 0264×0267×登录链交错 / 白名单包延迟语义×批量化 / 0225 长时计量 /
探针族常量折叠）——九面中七面闭合零新缺陷，两面发现并修复：

- **F1（0269，长期高负载内存面）**：0241/0242 的 chunk 包双缓存只标记失效不释放引用、
  无末观众驱逐——无人观看的已加载区块（出生点常驻/强载/卸载延迟尾迹）线性累积
  buffer(10-40KiB)+光照快照+memo DEFLATE 段（合计典型 40-115KiB/区块）直至卸载，
  多玩家大视距下 GB 级常驻（vanilla 为零）。修复：`moonrise$removeReceivedChunk`
  末观众归零时（主线程）清 holder 共享包+版本戳并同步释放 chunk 级缓存；在途批内
  发送持有各自不可变引用不受影响，新观众按版本门纯函数重建字节逐位一致。
  验证：applyPatches+compileJava+SmokeJoin 10/10+WalkGen 128s churn（数百次驱逐，
  tps=20.00 零错误）+ChurnStabilityBench 36 周期驱逐→重观看→重建 gate PASS。
- **F2-F4（PapoDiag jar 重编）**：报告分区 CME 连带丢失主线程栈→分节容错；报告文件
  病理停写下无界增长（~17MB/天）→8MiB 轮转上限；tick 历史索引 int 回绕（~3.4 年）
  抛 AIOOBE 终止心跳→看门狗永久伪报→floorMod。

判例：逐补丁审计闭环≠组合审计闭环（memo 属性协议与 locale 完备性的不变量只在族间
叠合处可证伪）；缓存类优化必须显式回答「无观众时何时释放」（version 门只保证正确性，
引用释放决定长期常驻规模）；诊断工具按「最坏时刻可用」标准设计。WalkGen harness
地形依赖判例（bot 固定 y 合成移动撞地形被 vanilla 反作弊踢出，非服务器缺陷）入
build.md 之外的本批次报告。详报：
note/report/perf/2026-09-02-crossfamily-audit-batch122.md
